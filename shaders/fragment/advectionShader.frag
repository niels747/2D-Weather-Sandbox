...
void main()
{
    ...
    if (wall[1] != 0) { // not wall
        ...
        base[3] = 1000.0; //special temperature, just to identify that this is a wall cell when drawing the graph
        ...
        if (wallX0Yp[1] != 0) { // cell above is not wall
            ...
            if (water[2] > 0.0 && tempC > 0.0) { // water evaporating from ground
                ...
                float evaporation = max((maxWater(CtoK(tempC)) - water[0]) * 0.00001, 0.); // water evaporating from land
                water[2] -= evaporation;
            }
    ...
    if (userInputValues.x < -0.5) { // whole width brush
        ...
        vec2 distFromMouse = vec2(absHorizontalDist(userInputValues.x, texCoord.x), userInputValues.y - texCoord.y);
