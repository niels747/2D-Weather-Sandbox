...
uniform sampler2D precipFeedbackTex; // [0] droplet weigth force,  [1] heating and cooling of fluid,  [2] evaporation and taking water from cloud
...
layout(location = 2) out ivec2 wall; // [0] walltype    [1] distance to nearest wall      [2] height above ground
...
void main()
{
    ...
    if (wall[1] != 0) { // is fluid, not wall
        ...
        water[1] = max(water[0] - maxWater(realTemp), 0.0); // recalculate cloud water after changing water

        water[2] = max(water[2] * 0.998 - 0.00005 + -precipFeedback[0] * 0.008, 0.0); // 0.004 for rain visualisation

        // remove smoke
        water[3] /= 1. + max(-precipFeedback[2] * 0.3, 0.0) - precipFeedback[0] * 0.003; // rain formation in clouds removes smoke quickly , falling rain slower
        water[3] += precipFeedback[0] * 0.0002; // falling rain slowly removes smoke linearly to remove last little bit
...
#define gravMult 0.0001 // 0.0002
        ...
        //float gravityForce = ((base[3] + baseX0Yp[3]) * 0.5 - (initial_T[int(fragCoord.y)] + initial_T[int(fragCoord.y) + 1]) * 0.5) * gravMult; // 0.0005  0.0001  gravity for convection
        ...
        if (nextToWall) {
            ...
            base.xy += vec2(vortForceX0Y0.x + vortForceX0Ym.x, vortForceX0Y0.y + vortForceXmY0.y) * vorticity; // apply vorticity force
            //}
        ...
        if (wall[1] <= wallInfluence) { // within range of wall
            ...
            if (wall[0] == 1) { // land surface
                base[3] += sunIntensity * lightHeatingConst * light[0] * cos(sunAngle) / (1. + snowCover) / devider; // sun heating land

                float evaporation = max((maxWater(realTemp) - water[0]) * landEvaporation / devider, 0.); // water evaporating from land
                ...
                water[0] += max((maxWater(waterTemperature) - water[0]) * waterEvaporation / devider, 0.); // water evaporating
