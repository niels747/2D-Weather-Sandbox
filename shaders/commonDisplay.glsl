...
vec4 tempToColor(float tempK, sampler2D colorScalesTex)
{
  float t = map_range(KtoC(tempK), 0.0, 1.0, 71.0, 72.0);
  t /= float(textureSize(colorScalesTex, 0).y); // 131.
  return texture(colorScalesTex, vec2(0., t));
}
