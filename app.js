...
async function mainScript(initialBaseTex, initialWaterTex, initialWallTex, initialRainDrops) {
	...
	function setupDatGui(strGuiControls) {
		...
		UI_folder.add(guiControls, "tool", { Temperature: "TOOL_TEMPERATURE", "Water Vapor / Cloud": "TOOL_WATER", Land: "TOOL_WALL_LAND", "Lake / Sea": "TOOL_WALL_SEA", Fire: "TOOL_WALL_FIRE", Smoke: "TOOL_SMOKE", Moisture: "TOOL_WALL_MOIST", Snow: "TOOL_WALL_SNOW" }).name("Tool").listen();
		...
		draw: function (simXpos, simYpos) {
			...
			function drawIsotherms() {
				...
				for (var T = -80.0; T <= 40.0; T += 10.0) {
					...
					if (T >= -30.0) c.fillText(printTemp(Math.round(T)), T_to_Xpos(T, graphBottem) - 20, this.graphCanvas.height - 5);
				}
				c.lineWidth = 1.0;
				c.stroke();
	...
	document.addEventListener("keydown", (event) => {
		if (event.keyCode == 17 || event.keyCode == 224) {
			...
			bPressed = true;
			if (new Date().getTime() - lastBpressTime < 300) guiControls.wholeWidth = !guiControls.wholeWidth; // toggle whole width brush
	...
	gl.uniform1f(gl.getUniformLocation(boundaryProgram, "sunIntensity"), guiControls.sunIntensity); // can be changed by GUI input
	...
	gl.uniform1f(gl.getUniformLocation(lightingProgram, "sunIntensity"), guiControls.sunIntensity);
	...
	function draw() {
		...
		} else if (inputType == 0 && !bPressed && (guiControls.tool == "TOOL_TEMPERATURE" || guiControls.tool == "TOOL_WATER")) {
			// cursor off
		...
		if (guiControls.displayMode == "DISP_REAL") {
			...
			if (guiControls.displayMode == "DISP_TEMPERATURE") {
				...
				gl.uniform3f(gl.getUniformLocation(temperatureDisplayProgram, "cursor"), mouseXinSim_SIG, mouseYinSim, guiControls.brushSize * 0.5);
				...
				gl.uniform3f(gl.getUniformLocation(IRtempDisplayProgram, "cursor"), mouseXinSim_SIG, mouseYinSim, guiControls.brushSize * 0.5);
				...
				gl.uniform3f(gl.getUniformLocation(universalDisplayProgram, "cursor"), mouseXinSim_SIG, mouseYinSim, guiControls.brushSize * 0.5);
	...
	async function prepareDownload() {
		...
		if (newFileName != null) {
			if (newFileName != "" && !newFileName.includes(".")) {
				...
				let compressedBlob = new Blob([Uint32Array.of(saveFileVersionID), compressed], { type: "application/x-binary" }); // turn back into blob and add version id in front
