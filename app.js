...
const saveFileVersionID = 368164894; // Uint32 id to check if save file is compatible
...
async function loadData() {
	...
	if (file) {
		...
		if (version == saveFileVersionID) {
			...
			let fileArrBuf = await file.slice(4).arrayBuffer(); // slice from behind version id to the end of the file
...
class LoadingBar {
	...
	constructor(percentIn) {
		if (percentIn == null) this.percent = 0;
		else this.percent = percentIn;
	...
	#update() {
		return new Promise((resolve) => {
			...
			let timeout;
			if (this.percent == 100) timeout = 5;
			else timeout = 5; // 50 for nicer feel
			setTimeout(() => {
				resolve();
			}, timeout);
		});
...
async function mainScript(
	...
	function setupDatGui(strGuiControls) {
		...
		guiControls.resetSettings = function () {
			if (confirm("Are you sure you want to reset all settings to default?")) {
				...
				gl.uniform1f(
					gl.getUniformLocation(velocityProgram, "wind"),
					guiControls.wind
				);
		...
		UI_folder.add(guiControls, "intensity", 0.005, 0.05).name(
			"Brush Intensity"
		);
		...
		radiation_folder
			.add(guiControls, "timeOfDay", 0.0, 23.9)
			.onChange(function () {
				updateSunlight();
			})
			.name("Time of day")
		...
		radiation_folder
			.add(guiControls, "dayNightCycle")
			.name("Day/Night Cycle")
			.listen();

		radiation_folder
			.add(guiControls, "latitude", -90.0, 90.0)
			.onChange(function () {
				updateSunlight();
			})
			.name("Latitude")
		...
		radiation_folder
			.add(guiControls, "month", 1.0, 12.9)
			.onChange(function () {
				updateSunlight();
			})
			.name("Month")
		...
		radiation_folder
			.add(guiControls, "sunAngle", -10.0, 190.0)
			.onChange(function () {
		...
		precipitation_folder
			.add(guiControls, "subZeroThreshold", 0.1, 2.0)
			.onChange(function () {
				gl.useProgram(precipitationProgram);
				gl.uniform1f(
					gl.getUniformLocation(precipitationProgram, "subZeroThreshold"),
		...
		precipitation_folder
			.add(guiControls, "spawnChance", 0.00001, 0.0001)
			.onChange(function () {
				gl.useProgram(precipitationProgram);
				gl.uniform1f(
					gl.getUniformLocation(precipitationProgram, "spawnChance"),
		...
		precipitation_folder
			.add(guiControls, "snowDensity", 0.1, 1.0)
			.onChange(function () {
				gl.useProgram(precipitationProgram);
				gl.uniform1f(
					gl.getUniformLocation(precipitationProgram, "snowDensity"),
		...
		precipitation_folder
			.add(guiControls, "fallSpeed", 0.0001, 0.0006)
			.onChange(function () {
				gl.useProgram(precipitationProgram);
				gl.uniform1f(
					gl.getUniformLocation(precipitationProgram, "fallSpeed"),
		...
		precipitation_folder
			.add(guiControls, "growthRate0C", 0.00001, 0.001)
			.onChange(function () {
				gl.useProgram(precipitationProgram);
				gl.uniform1f(
					gl.getUniformLocation(precipitationProgram, "growthRate0C"),
		...
		precipitation_folder
			.add(guiControls, "growthRate_30C", 0.0005, 0.001)
			.onChange(function () {
				gl.useProgram(precipitationProgram);
				gl.uniform1f(
					gl.getUniformLocation(precipitationProgram, "growthRate_30C"),
		...
		precipitation_folder
			.add(guiControls, "inactiveDroplets", 0, NUM_DROPLETS)
			.listen()
			.name("Inactive Droplets");
		precipitation_folder
			.add(guiControls, "showDrops")
			.name("Show Droplets")
			.listen();
		...
		datGui
			...
			.listen();
		datGui
			.add(guiControls, "exposure", 1.0, 10.0)
			.onChange(function () {
				gl.useProgram(realisticDisplayProgram);
				gl.uniform1f(
					gl.getUniformLocation(realisticDisplayProgram, "exposure"),
	...
	var soundingGraph = {
		...
		draw: function (simXpos, simYpos) {
			...
			gl.readPixels(
				...
				gl.FLOAT,
				baseTextureValues
			); // read a vertical culumn of cells
			...
			for (var y = 0; y < sim_res_y; y++) {
				...
				var temp =
					potentialTemp -
					((y / sim_res_y) * guiControls.simHeight * guiControls.dryLapseRate) /
						1000.0 -
					273.15;
				...
				if (y == simYpos) {
					c.fillText(
						"" +
							printAltitude(
								Math.round(
									map_range(y - 1, 0, sim_res_y, 0, guiControls.simHeight)
								)
							),
						5,
						scrYpos + 5
			...
			function drawIsotherms() {
				...
				for (var T = -80.0; T <= 40.0; T += 10.0) {
					...
					if (T >= -30.0)
						c.fillText(
							printTemp(Math.round(T)),
							T_to_Xpos(T, graphBottem) - 20,
	...
	function logSample() {
		...
		console.log(
			"Temp :",
			KtoC(potentialToRealT(baseTextureValues[3], simYpos)).toFixed(2) + " °C"
		);
		...
		console.log(
			"IR down:   ",
			lightTextureValues[2].toFixed(2),
			"W/m²",
			KtoC(IR_temp(lightTextureValues[2])).toFixed(2) + " °C"
		);
		console.log(
			"IR up:     ",
			lightTextureValues[3].toFixed(2),
			"W/m²",
			KtoC(IR_temp(lightTextureValues[3])).toFixed(2) + " °C"
		);
		console.log(
			"Net IR up: ",
			(lightTextureValues[3] - lightTextureValues[2]).toFixed(2),
			"W/m²"
		);
	}
			...
			if (now - lastWheel > 20) {
				...
				if (changeViewZoom(1.0 + delta)) {
					...
					var mousePositionZoomCorrectionX =
						(((mouseX - canvas.width / 2 + viewXpos) * delta) /
							viewZoom /
							canvas.width) *
						2.0;
					var mousePositionZoomCorrectionY =
						((((mouseY - canvas.height / 2 + viewYpos) * delta) /
							viewZoom /
							canvas.height) *
							2.0) /
						canvas_aspect;
	...
	const precipDisplayVertexShader = await loadShader(
		"precipDisplayShader.vert"
	);
	...
	const temperatureDisplayShader = await loadShader(
		"temperatureDisplayShader.frag"
	);
	const precipDisplayShader = await loadShader("precipDisplayShader.frag");
	const universalDisplayShader = await loadShader(
		"universalDisplayShader.frag"
	);
	const skyBackgroundDisplayShader = await loadShader(
		"skyBackgroundDisplayShader.frag"
	);
	const realisticDisplayShader = await loadShader(
		"realisticDisplayShader.frag"
	);
	...
	const temperatureDisplayProgram = createProgram(
		dispVertexShader,
		temperatureDisplayShader
	);
	const precipDisplayProgram = createProgram(
		precipDisplayVertexShader,
		precipDisplayShader
	);
	const universalDisplayProgram = createProgram(
		dispVertexShader,
		universalDisplayShader
	);
	const skyBackgroundDisplayProgram = createProgram(
		realDispVertexShader,
		skyBackgroundDisplayShader
	);
	const realisticDisplayProgram = createProgram(
		realDispVertexShader,
		realisticDisplayShader
	);
	const IRtempDisplayProgram = createProgram(
		dispVertexShader,
		IRtempDisplayShader
	);
	...
	var positionAttribLocation = gl.getAttribLocation(
		pressureProgram,
		"vertPosition"
	); // 0 these positions are the same for every program, since they all use the same vertex shader
	...
	gl.texImage2D(
		...
		gl.FLOAT,
		null
	); // HALF_FLOAT before, but problems with acuracy
	...
	for (var y = 0; y < sim_res_y + 1; y++) {
		var realTemp = Math.max(map_range(y, 0, sim_res_y + 1, 15.0, -70.0), -60); // almost standard atmosphere
		//	if (y < sim_res_y * 0.15) {
	...
	gl.uniform2f(
		gl.getUniformLocation(setupProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	gl.uniform2f(
		gl.getUniformLocation(setupProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	...
	gl.uniform2f(
		gl.getUniformLocation(advectionProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform1fv(
		gl.getUniformLocation(advectionProgram, "initial_T"),
		initial_T
	);
	...
	gl.uniform2f(
		gl.getUniformLocation(velocityProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	gl.uniform1f(
	...
	gl.uniform2f(
		gl.getUniformLocation(boundaryProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(boundaryProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	gl.uniform1f(
	...
	gl.useProgram(lightingProgram);
	gl.uniform2f(
		gl.getUniformLocation(lightingProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(lightingProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	gl.uniform1f(
	...
	gl.uniform2f(
		gl.getUniformLocation(temperatureDisplayProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(temperatureDisplayProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	...
	gl.uniform1f(
		gl.getUniformLocation(temperatureDisplayProgram, "dryLapse"),
		dryLapse
	);

	gl.useProgram(precipDisplayProgram);
	gl.uniform2f(
		gl.getUniformLocation(precipDisplayProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(precipDisplayProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	...
	gl.uniform2f(
		gl.getUniformLocation(universalDisplayProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(universalDisplayProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	...
	gl.useProgram(realisticDisplayProgram);
	gl.uniform2f(
		gl.getUniformLocation(realisticDisplayProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(realisticDisplayProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	...
	gl.uniform1f(
		gl.getUniformLocation(realisticDisplayProgram, "dryLapse"),
		dryLapse
	);
	gl.uniform1f(
		gl.getUniformLocation(realisticDisplayProgram, "exposure"),
	...
	gl.uniform2f(
		gl.getUniformLocation(precipitationProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(precipitationProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	gl.uniform1f(
		gl.getUniformLocation(precipitationProgram, "dryLapse"),
		dryLapse
	);
	gl.uniform1f(
		gl.getUniformLocation(precipitationProgram, "evapHeat"),
	...
	gl.uniform2f(
		gl.getUniformLocation(IRtempDisplayProgram, "resolution"),
		sim_res_x,
		sim_res_y
	);
	gl.uniform2f(
		gl.getUniformLocation(IRtempDisplayProgram, "texelSize"),
		texelSizeX,
		texelSizeY
	);
	...
	gl.useProgram(skyBackgroundDisplayProgram);
	gl.uniform1i(
		gl.getUniformLocation(skyBackgroundDisplayProgram, "lightTex"),
		3
	);
	...
	function draw() {
		...
		if (SETUP_MODE) {
			...
			if (leftMousePressed) {
				...
				else if (guiControls.tool == "TOOL_WALL_MOIST") inputType = 14;
				else if (guiControls.tool == "TOOL_WALL_SNOW") inputType = 15;
			...
			gl.uniform1i(
				gl.getUniformLocation(advectionProgram, "userInputType"),
				inputType
			); // 0 = nothing 	1 = temp	 2 = wall	3 = heating wall	4 = cooling wall
			...
			if (!guiControls.paused) {
				...
				for (var i = 0; i < guiControls.IterPerFrame; i++) {
					...
					gl.useProgram(precipitationProgram);
					gl.uniform1f(
						gl.getUniformLocation(precipitationProgram, "frameNum"),
						IterNum
					);
					gl.enable(gl.BLEND);
		...
		if (guiControls.displayMode == "DISP_REAL") {
			...
			gl.uniform3f(
				gl.getUniformLocation(skyBackgroundDisplayProgram, "view"),
				viewXpos,
				viewYpos,
				viewZoom
			);
			...
			gl.uniform3f(
				gl.getUniformLocation(realisticDisplayProgram, "view"),
				viewXpos,
				viewYpos,
				viewZoom
			);
			...
			if (guiControls.showDrops) {
				...
				gl.uniform3f(
					gl.getUniformLocation(precipDisplayProgram, "view"),
					viewXpos,
					viewYpos,
					viewZoom
				);
			...
			if (guiControls.displayMode == "DISP_TEMPERATURE") {
				...
				gl.uniform3f(
					gl.getUniformLocation(temperatureDisplayProgram, "view"),
					viewXpos,
					viewYpos,
					viewZoom
				);
				...
				gl.uniform3f(
					gl.getUniformLocation(IRtempDisplayProgram, "view"),
					viewXpos,
					viewYpos,
					viewZoom
				);
				...
				gl.uniform3f(
					gl.getUniformLocation(universalDisplayProgram, "view"),
					viewXpos,
					viewYpos,
					viewZoom
				);
				...
				switch (guiControls.displayMode) {
					case "DISP_HORIVEL":
						...
						gl.uniform1f(
							gl.getUniformLocation(universalDisplayProgram, "dispMultiplier"),
							10.0
						); // 20.0
						break;
					case "DISP_VERTVEL":
					...
					case "DISP_WATER":
						...
						gl.uniform1f(
							gl.getUniformLocation(universalDisplayProgram, "dispMultiplier"),
							-0.06
						); // negative number so positive amount is blue
						break;
					case "DISP_IRHEATING":
						gl.activeTexture(gl.TEXTURE0);
						gl.bindTexture(gl.TEXTURE_2D, lightTexture_0);
	...
	async function prepareDownload() {
		var newFileName = prompt(
			"Please enter a file name. Can not include '.'",
			saveFileName
		);
		...
		if (newFileName != null) {
			if (newFileName != "" && !newFileName.includes(".")) {
				...
				let precipBufferValues = new ArrayBuffer(
					rainDrops.length * Float32Array.BYTES_PER_ELEMENT
				);
				gl.bindBuffer(gl.ARRAY_BUFFER, precipVertexBuffer_0);
				gl.getBufferSubData(
					gl.ARRAY_BUFFER,
					0,
					new Float32Array(precipBufferValues)
				);
	...
	function loadSourceFile(fileName) {
		...
		else if (request.status === 404) throw "File not found: " + fileName;
		else throw "File loading error" + request.status;
	}
	...
	function adjIterPerFrame(adj) {
		guiControls.IterPerFrame = Math.round(
			Math.min(Math.max(guiControls.IterPerFrame + adj, 1), 50)
		);
	}
